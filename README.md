...
In addition to raw Markdown exports, the CLI can now generate **CEO-ready DOCX and PDF** documents directly from stage files:
...
### Sequential questions (v1.8)

From version **1.8**, the agent uses a **sequential questioning protocol** inside each stage:
...
You can control the flow using high-level commands described in the system prompt:
...
From version **1.8**, you can additionally generate **formatted DOCX and PDF** outputs from these Markdown files using the CLI:

```bash
cd BABOK_ANALYST

# Example for project BABOK-20260208-XXXX
babok make all BABOK-20260208-XXXX
```

This produces CEO‑ready documents (DOCX + PDF) in the project’s `exports/` directory, based on the stage `.md` files.
