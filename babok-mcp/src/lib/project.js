...
// Default-profile constants; kept for backward compatibility with existing imports.
const DEFAULT_PROFILE = loadProfile(DEFAULT_PROFILE_ID);
export const STAGES = getStages(DEFAULT_PROFILE);
export const STAGE_FILE_NAMES = getStageFileNames(DEFAULT_PROFILE);
export const STAGE_PROMPT_FILE_NAMES = getStagePromptFileNames(DEFAULT_PROFILE);

/** Coerce a profile id / object / undefined into a profile object. */
function toProfile(profile) {
  if (!profile) return DEFAULT_PROFILE;
  return typeof profile === 'string' ? loadProfile(profile) : profile;
}
...
function findProjectsDirUpward(startDir, maxDepth = 8) {
  ...
  for (let depth = 0; depth < maxDepth; depth += 1) {
    for (const rel of ['projects', path.join('BABOK_ANALYST', 'projects')]) {
      ...
      const hasProject = fs.readdirSync(candidate).some(
        (name) => idRe.test(name) && fs.statSync(path.join(candidate, name)).isDirectory(),
      );
      if (hasProject) return candidate;
    }
...
/**
 * Resolve the stage prompt directory for a profile.
 ...
 * Other profiles: profile.paths.stages_dir (BABOK_AGENT_DIR is not applied).
 * @param {string|object} [profile]
 */
export function getAgentStagesDir(profile) {
 ...
 */
export function generateProjectId(profile) {
  const p = toProfile(profile);
  const now = new Date();
...
/** Read stage prompt file content for a profile, returns null if not found */
export function getStagePrompt(stageN, profile) {
  const p = toProfile(profile);
  const stagesDir = getAgentStagesDir(p);
  if (!stagesDir) return null;
  const filename = getStage(p, stageN)?.prompt_file;
...
export function getStageFileName(stageN, profile) {
  return getStage(toProfile(profile), stageN)?.deliverable_file ?? null;
}

/** Read a stage deliverable MD file, returns null if not found */
export function getDeliverable(projectId, stageN, profile) {
  const filename = getStageFileName(stageN, profile);
  if (!filename) return null;
