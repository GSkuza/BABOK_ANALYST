 ...
 *
 * This file is dependency-free on purpose: it is mirrored byte-for-byte into
 * babok-mcp/src/lib/model-routing.js (enforced by tests/unit/lib-parity.test.js).
 */
import fs from 'fs';
...
/**
 * Minimal provider catalog used for routing. Must stay aligned with PROVIDERS
 * in cli/src/llm.js (tests/unit/model-routing.test.js checks this).
 */
export const ROUTING_PROVIDERS = {
  gemini: { name: 'Google Gemini', envKey: 'GEMINI_API_KEY', defaultModel: 'gemini-2.0-flash' },
  openai: { name: 'OpenAI', envKey: 'OPENAI_API_KEY', defaultModel: 'gpt-5.6-terra' },
  anthropic: { name: 'Anthropic Claude', envKey: 'ANTHROPIC_API_KEY', defaultModel: 'claude-sonnet-5' },
  huggingface: { name: 'Hugging Face', envKey: 'HF_API_KEY', defaultModel: 'Qwen/Qwen2.5-72B-Instruct' },
  vertex: { name: 'Google Vertex AI', envKey: 'VERTEX_PROJECT_ID', defaultModel: 'gemini-2.5-pro-exp-03-25' },
  local: { name: 'Local / SGLang (OpenAI-compatible)', envKey: 'LOCAL_API_KEY', defaultModel: 'speakleash/Bielik-11B-v3.0-Instruct' },
};

const PROVIDERS = ROUTING_PROVIDERS;
const KEYSTORE_FILE_NAME = '.babok_keystore';
...
function ruleLevel(routing, { profile, stage } = {}, create = false) {
  if (stage !== undefined && stage !== null && !profile) {
    throw new ModelRoutingError('A stage rule requires a profile.');
  }
  if (!profile) return { get: () => routing.default, set: (rule) => { routing.default = rule; } };
  if (!routing.profiles[profile]) {
    if (!create) return null;
    routing.profiles[profile] = { default: {}, stages: {} };
  }
  const entry = routing.profiles[profile];
  if (stage === undefined || stage === null) {
    return { get: () => entry.default, set: (rule) => { entry.default = rule; } };
  }
  const key = String(stage);
  return {
    get: () => entry.stages[key] || {},
    set: (rule) => { entry.stages[key] = rule; },
  };
}

/**
 * Return a new routing document with one rule patched. In `patch`, `undefined`
 * keeps the current value and `null` removes it (inherit from the parent level).
 * Setting a new provider without a model clears a model chosen for another provider.
 *
 * @param {object} routing
 * @param {{ profile?: string|null, stage?: number|null }} target
 * @param {{ provider?: string|null, model?: string|null, temperature?: number|null, effort?: string|null, fallbacks?: Array|null }} patch
 */
export function setModelRoutingRule(routing, target, patch = {}, options = {}) {
  const next = normalizeModelRouting(routing, options);
  const level = ruleLevel(next, target, true);
  const rule = { ...level.get() };
  if (patch.provider !== undefined) {
    if (patch.provider !== rule.provider) delete rule.model;
    if (patch.provider === null) delete rule.provider;
    else rule.provider = patch.provider;
  }
  for (const field of ['model', 'temperature', 'effort', 'fallbacks']) {
    if (patch[field] === undefined) continue;
    if (patch[field] === null) delete rule[field];
    else rule[field] = patch[field];
  }
  if (!rule.provider) delete rule.model;
  level.set(rule);
  return normalizeModelRouting(next, options);
}

/** Return a new routing document without the rule at `target` (a whole profile when no stage is given). */
export function removeModelRoutingRule(routing, target = {}, options = {}) {
  const next = normalizeModelRouting(routing, options);
  if (!target.profile) {
    if (target.stage !== undefined && target.stage !== null) throw new ModelRoutingError('A stage rule requires a profile.');
    next.default = {};
  } else if (target.stage === undefined || target.stage === null) {
    delete next.profiles[target.profile];
  } else if (next.profiles[target.profile]) {
    delete next.profiles[target.profile].stages[String(target.stage)];
  }
  return normalizeModelRouting(next, options);
}

/** True when the routing document contains at least one explicit override. */
export function hasModelRoutingRules(routing) {
  if (!routing) return false;
  return routing.failover_all_providers === true
    || Object.keys(routing.default || {}).length > 0
    || Object.keys(routing.profiles || {}).length > 0;
}
...
function readDotEnv(baseDir) {
  try {
    return fs.readFileSync(path.join(baseDir, '.env'), 'utf-8');
  } catch {
    return '';
  }
}
...
/**
 * Detect which providers have credentials configured WITHOUT decrypting any key:
 * environment variable, `.env` entry or an entry in `.babok_keystore` under `baseDir`.
 *
 * @param {{ baseDir?: string, env?: object, providers?: object }} [options]
 * @returns {{ providers: string[], preferredProvider: string|null }}
 */
export function detectConfiguredProviders(options = {}) {
  const providers = options.providers || PROVIDERS;
  const env = options.env || process.env;
  const baseDir = options.baseDir || process.cwd();
  const keystore = readJsonFile(path.join(baseDir, KEYSTORE_FILE_NAME)) || {};
  const dotEnv = readDotEnv(baseDir);
  const configured = Object.keys(providers).filter((provider) => {
    const envKey = providers[provider].envKey;
    if (envKey && env[envKey]) return true;
    if (provider !== 'vertex' && envKey && new RegExp(`^${envKey}\\s*=\\s*\\S+`, 'm').test(dotEnv)) return true;
    return Boolean(keystore[provider]?.k);
  });
  const preferred = keystore._preferred_provider;
  return {
    providers: configured,
    preferredProvider: providers[preferred] ? preferred : null,
  };
}

/** Human-readable multi-line summary of a resolved route (no credentials). */
export function describeModelRoute(route, providers = PROVIDERS) {
  const lines = [];
  const where = route.stage === null ? `profile ${route.profile}` : `profile ${route.profile}, stage ${route.stage}`;
  lines.push(`Route for ${where}:`);
  if (route.candidates.length === 0) {
    lines.push('  (no configured provider — add an API key first)');
  }
  route.candidates.forEach((candidate, index) => {
    const name = providers[candidate.provider]?.name || candidate.provider;
    lines.push(`  ${index + 1}. ${name} · ${candidate.model}  [${candidate.source}]`);
  });
  lines.push(`  temperature: ${route.temperature ?? 'provider default'} [${route.sources.temperature}]`);
  lines.push(`  effort:      ${route.effort ?? 'provider default'} [${route.sources.effort}]`);
  if (route.skipped.length) {
    lines.push(`  skipped (no API key): ${route.skipped.map(c => `${c.provider}/${c.model}`).join(', ')}`);
  }
  return lines.join('\n');
}
